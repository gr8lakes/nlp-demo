...
def classification_analyze(client, text):
    ...
    score = format(
        Decimal(response.payload[0].classification.score).quantize(Decimal('0.00')))
...
def import_to_bq(client, table_id, uri):
    ...
    job_config.schema = [
        ...
        bigquery.SchemaField("classification_topic",
        ...
        bigquery.SchemaField("entity_sentiment_score",
                             "FLOAT", mode="REPEATED"),
        bigquery.SchemaField("entity_sentiment_magnitude",
                             "FLOAT", mode="REPEATED"),
    ]
...
if __name__ == '__main__':
    ...
    file = 'analyze_results' + real_time + '.json'
    g = open(file, 'a')
    ...
    try:
        ...
        # upload file to GCS
        uri = upload_to_bucket("test-chent-we", file, file)

        print("Import to BigQuery Table from json file in GCS")
        table = project_id + '.' + dataset_id + '.' + 'analyze_result' + real_time

        import_to_bq(bq_client, table, uri)
        print("finished!")
